...
import mao.console_test_client.thread.BizThread;
import mao.console_test_client.thread.StressTestBizThread;
    ...
    protected void channelRead0(ChannelHandlerContext ctx, LoginResponseMessage loginResponseMessage) throws Exception
    {
        try
        {
            if (loginResponseMessage.isSuccess())
            {
                ...
                MainApplication.setLoginUsername(loginResponseMessage.getUsername());
                BizThread bizThread = new BizThread(StressTestClient.getChannel(), loginResponseMessage.getUsername());
                StressTestBizThread stressTestBizThread =
                        new StressTestBizThread(StressTestClient.getChannel(), loginResponseMessage.getUsername());
                //强行结束
                StressTestClient.getThread().stop();
                StressTestClient.setThread(stressTestBizThread);
                //启动业务线程
                //bizThread.start();
                stressTestBizThread.start();
            }
            else
            {
                //登录失败
